...
void main() {
  group("Transaction", () {
    ...
    expectInputSignedSize(Input input) =>
        expect(input.size, lessThanOrEqualTo(input.signedSize!));
    ...
    test("sign() failure", () {
      ...
      expect(
        () => tx.sign(inputN: 2, key: tweakedKey),
        throwsA(isA<CannotSignInput>()),
      );
      ...
      expect(
        () => tx.sign(inputN: 2, key: privkey, prevOuts: prevOuts),
        throwsA(isA<CannotSignInput>()),
      );
    ...
    test("sign key-path P2TR input", () {
      ...
      final signed = tx
          .sign(inputN: 0, key: tweakedPriv, prevOuts: prevOuts)
          .sign(
            inputN: 1,
            key: tweakedPriv,
            prevOuts: prevOuts,
    ...
    test("sign P2SH multisig and add inputs/outputs", () {
      ...
      expectMultisigSigs(signed.addOutput(exampleOutput), [
        SigHashType.single(),
        SigHashType.none(),
      ]);
    });
