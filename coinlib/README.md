...
The secp256k1-coinlib sources are vendored under `third_party/secp256k1` and
compiled by the build hook in `hook/build.dart`. Bindings for the native
libraries (excluding WebAssembly) are generated from the vendored headers using
`dart run ffigen` within the `coinlib` package.
